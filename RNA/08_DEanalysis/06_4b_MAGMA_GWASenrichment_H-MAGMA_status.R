...
for (trait in traits){
  ...
  files <- list.files(path = paste0(basedir, "tables/GWASenrichment/MAGMAoutput/status/"),
                      pattern = p,
                      full.names = FALSE)
  # read in the files
  magma_score_list <- list()
  for (f in files){
    m <- read.table(paste0(basedir, "tables/GWASenrichment/MAGMAoutput/status/", f),
                  skip = 3, header = TRUE)
...
ggsave(filename = paste0(basedir, "plots/06_4_MAGMA_GWASenrichment_H-MAGMA/", 
                         "/06_4_MAGMA_GWASenrichment_H-MAGMA_DE_status.pdf"),
       width = 8,
       height = 6)
