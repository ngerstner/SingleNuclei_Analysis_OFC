...
## with DESeq2 on full pseudobulk data
...
metadata <- metadata[metadata$cluster_id == "Astro_FB",] # rows are redundant
...
# Plot histogram of counts (pseudobulk)
hist(t(counts_sum))
max(counts_sum)
min(counts_sum)
median(t(counts_sum))
...
dds <- DESeqDataSetFromMatrix(t(counts_sum), 
                              colData = metadata, 
...
# Remove outliers
dds <- outlier_removal(dds,paste0(basedir, "plots/outlier_removal/PseudobulkComplete", 
                                 "/03_3_PseudobulkComplete"))
...
levels(metadata$Status)[2]
levels(metadata$Status)[1]

# get results
contrast <- c("Status", levels(metadata$Status)[2], levels(metadata$Status)[1])
res <- results(dds, 
               contrast = contrast,
               alpha = 0.1)
res <- lfcShrink(dds, 
...
write.csv(sig_res,
          paste0(basedir, "tables/DESeq2_", test, "/PseudobulkComplete/PseudobulkComplete_", 
                 des, "_sig", padj_cutoff, 
                 "_", test, "_pcFromCorrectedDataAllCellTypes.csv"))

# Write all genes to file
write.csv(res_tbl, 
          paste0(basedir, "tables/DESeq2_", test, "/PseudobulkComplete/PseudobulkComplete_", 
                 des, "_", test, "_pcFromCorrectedDataAllCellTypes.csv"))
...
write.csv(meta_deseq,
          paste0(basedir, "tables/DESeq2_", test, "/PseudobulkComplete/PseudobulkComplete_", des,
...
tophit_boxplots <- function(g, gene_name, data = c("raw", "norm", "corr")){
  ...
  ggsave(filename = paste0(basedir, "plots/03_3_DESeq2_PseudobulkComplete_tophits/",
                           gene_name, "_", data, ".pdf"),
         width = 8,
         height = 6)
...
if (plot_th) {
  ...
  if (nrow(sig_res) >= 5){
    tophit_boxplots(sig_res$gene[5], sig_res$X[5], data = "corr")
    tophit_boxplots(sig_res$gene[5], sig_res$X[5], data = "norm")
    tophit_boxplots(sig_res$gene[5], sig_res$X[5], data = "raw")
  }
