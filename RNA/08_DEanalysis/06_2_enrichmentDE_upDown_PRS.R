...
for (d in c("up", "down")){
  ...
  for (ct in names(de_list)){
    ...
    ggsave(filename = paste0(basedir, "plots/06_2_DEenrichments_upDown/", 
                             prs_trait,"/06_2_",ct,"_GOenrichment_",d,"_DE_PRS_",prs_trait,".pdf"))
    ...
    x[1:20,] %>%
      ...
      coord_flip()
    ggsave(filename = paste0(basedir, "plots/06_2_DEenrichments_upDown/", 
                             prs_trait,"/06_2_",ct,"_DiseaseEnrichment_",d,"_DE_PRS_",prs_trait,".pdf"))
    ...
    kk[1:20,] %>%
      ...
      coord_flip()
    ggsave(filename = paste0(basedir, "plots/06_2_DEenrichments_upDown/", 
                             prs_trait,"/06_2_",ct,"_KEGGEnrichment_",d,"_DE_PRS_",prs_trait,".pdf"))
...
for (d in c("up", "down")){
  ...
  write.csv(df_list[[d]][["go_df"]], paste0(basedir, "tables/KEGGenrichment/", prs_trait, "/",
  ...
  df_list[[d]][["kegg_df"]] <- bind_rows(enr[[d]][["kegg_enrich"]], .id="celltype") %>%
    ...
    pivot_wider(names_from = celltype,
                values_from = p.adjust) %>%
    filter(if_any(where(is.numeric), ~ . < 0.05)) 
  write.csv(df_list[[d]][["kegg_df"]], paste0(basedir, "tables/KEGGenrichment/", prs_trait, "/",
  ...
  df_list[[d]][["disease_df"]] <- bind_rows(enr[[d]][["disease_enrich"]], .id="celltype") %>%
    ...
    pivot_wider(names_from = celltype,
                values_from = p.adjust) %>%
    filter(if_any(where(is.numeric), ~ . < 0.05)) 
  write.csv(df_list[[d]][["disease_df"]], paste0(basedir, "tables/KEGGenrichment/", prs_trait, "/",
...
plot_heatmap <- function(x, d, enr_type){
  if(nrow(x) > 0){
    ...
    pdf(file = paste0(basedir, "plots/06_2_DEenrichments_upDown/", 
                      prs_trait,"/06_2_AllCellTypes_",enr_type,"_",d,"_DE_PRS_",prs_trait,".pdf"),
        width = 12, height = 10)
...
for (d in c("up", "down")){
  for(et in c("GOenrichment", "KEGGenrichment", "DiseaseEnrichment")){
    
    if (et == "GOenrichment"){
      plot_heatmap(df_list[[d]][["go_df"]], d, et)
    } else if (et == "KEGGenrichment") {
      plot_heatmap(df_list[[d]][["kegg_df"]], d, et)
    } else {
...
plot_KEGG_heatmap <- function(x_up, x_down, enr_type){
  ...
  for (d in c("up", "down")){
    ...
    pdf(file = paste0(basedir, "plots/06_2_DEenrichments_upDown/", 
                      prs_trait, "/06_2_AllCellTypes_",enr_type,"_",d,"_DE_", prs_trait, ".pdf"),
        width = 12, height = 10)
    if(nrow(mat) > 1){
