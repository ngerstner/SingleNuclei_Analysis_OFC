...
des <- "~ PRS + Sex + Age + Brain.pH + RIN + PMI + X6.Batch"
des_null <- "~ Sex + Age + Brain.pH + RIN + PMI + X6.Batch"
des_varPart <- "~ PRS + (1|Sex) + Age + Brain.pH + RIN + PMI + (1|X6.Batch)"
```
...
y2 <- removeBatchEffect(y, batch = d$samples$X6.Batch, batch2 = d$samples$Sex,
                        covariates = d$samples[,c("PRS", "Age", "RIN", "Brain.pH", "PMI")])
...
write.csv(pc$rotated[,c(1:10)],
          file = paste0(basedir, "tables/PCA_noise/", cluster, "_PCAnoise_rotated_PRS_",prs_trait,".csv"))
write.csv(pc$variance,
          file = paste0(basedir, "tables/PCA_noise/", cluster, "_PCAnoise_variance_PRS_",prs_trait,".csv"))
