...
p <- paste0("GeneScore_WaldTest_OutlierRemoved_Normalized_.*_", prs_trait)
...
de_down <- list()
for (f in files){
  de <- read.csv(paste0(DEA_dir, "tables/GeneScoreMatrix/PRSext/", f))
  ct <- sub(paste0("GeneScore_WaldTest_OutlierRemoved_Normalized_(.*)_",prs_trait,".csv"),"\\1",f)
...
for (d in c("up", "down")){
  ...
  for (ct in names(de_list)){
    ...
    mutate(Description = fct_reorder(Description, -p.adjust)) %>%
      ...
      coord_flip()
    ggsave(filename = paste0(DEA_dir, "plots/06_1_EnrichmentAnalysis_upDown/", prs_trait, "/",
                          "06_1_",ct,"_GOEnrichment_",d,"_DE_", prs_trait, ".pdf"))
    ...
    x[1:20,] %>%
      ...
      coord_flip()
    ggsave(filename = paste0(DEA_dir, "plots/06_1_EnrichmentAnalysis_upDown/", prs_trait, "/",
                          "06_1_",ct,"_DiseaseEnrichment_",d,"_DE_", prs_trait, ".pdf"))
    ...
    kk[1:20,] %>%
      ...
      coord_flip()
    ggsave(filename = paste0(DEA_dir, "plots/06_1_EnrichmentAnalysis_upDown/", prs_trait, "/",
                          "06_1_",ct,"_KEGGEnrichment_",d,"_DE_", prs_trait, ".pdf"))
...
for (d in c("up", "down")){
  ...
  write.csv(df_list[[d]][["go_df"]], paste0(DEA_dir, "tables/KEGGenrichment/", prs_trait, "/",
  ...
  df_list[[d]][["kegg_df"]] <- bind_rows(enr[[d]][["kegg_enrich"]], .id="celltype") %>%
    ...
    pivot_wider(names_from = celltype,
                values_from = p.adjust) %>%
    filter(if_any(where(is.numeric), ~ . < 0.05)) 
  write.csv(df_list[[d]][["kegg_df"]], paste0(DEA_dir, "tables/KEGGenrichment/", prs_trait, "/",
  ...
  df_list[[d]][["disease_df"]] <- bind_rows(enr[[d]][["disease_enrich"]], .id="celltype") %>%
    ...
    pivot_wider(names_from = celltype,
                values_from = p.adjust) %>%
    filter(if_any(where(is.numeric), ~ . < 0.05))
  write.csv(df_list[[d]][["disease_df"]], paste0(DEA_dir, "tables/KEGGenrichment/", prs_trait, "/",
...
plot_heatmap <- function(x, d, enr_type){
  if(nrow(x) > 0){
    ...
    pdf(file = paste0(DEA_dir, "plots/06_1_EnrichmentAnalysis_upDown/", prs_trait,
                      "/06_1_AllCellTypes_",enr_type,"_",d,"_DE_", prs_trait, ".pdf"),
        width = 12, height = 10)
...
plot_KEGG_heatmap <- function(x_up, x_down, enr_type){
  ...
  for (d in c("up", "down")){
    ...
    pdf(file = paste0(DEA_dir, "plots/06_1_EnrichmentAnalysis_upDown/", prs_trait, 
                      "/06_1_AllCellTypes_",enr_type,"_",d,"_DE_", prs_trait, ".pdf"),
        width = 12, height = 10)
    cluster_col_var <- ifelse(nrow(mat)>1, TRUE, FALSE)
    pheatmap(as.matrix(mat),
            display_numbers = mat_sig,
            cluster_rows = FALSE,
            cluster_cols = cluster_col_var,
